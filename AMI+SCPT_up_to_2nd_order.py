...
for U in [1,2,3,4,5,6]:
  for beta in [5,4,3,2,1]:
    for n in range (0,10):
      ...
      SIGMA_0=0
      SIGMA=0
      file_name='AMI+SCPT_with_feed_up_to_2nd_vs_iteration_n='+str(n)+'_U='+str(U)+'_beta='+str(beta)+'_pi_half_pi_half_MC=1e6.dat'
      file = open('result_AMI+SCPT_up_to_second_with_feedback_iter_1e6/'+file_name, 'w')  
      ...
      for i in range(0,50):
        ...
        if i==0:
          ...
          er_SIGMA_1 = out_second_order[1]*1.0j 
        else:
          SIGMA_1 = first_order[0]*1.0j + first_order[2] + out_second_order[0]*1.0j + out_second_order[2]
          er_SIGMA_1 = 1j*first_order[1] + first_order[3] + out_second_order[1]*1.0j + out_second_order[3]
