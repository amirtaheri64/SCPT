...
for U in [1,2,3,4]:
  for beta in [1,2,3,4,5]:
    for n in range (0,2):
      ...
      SIGMA_0=0
      SIGMA=0
      file_name='SCPT_with_feed_up_to_2nd_vs_iteration_pi_0_mu_'+str(mu)+'_n_'+str(n)+'_U='+str(U)+'_beta='+str(beta)+'_pi_half_pi_half_MC=1e6.dat'
      file = open('result_SCPT_up_to_second_with_feedback_mu=1_pi_0/'+file_name, 'w')  
      step=1000000
      for i in range(0,50):
        print i+1
        ...
        print
        if i==0:
          SIGMA_1 = first_order[0].real + out_second_order[0]*1.0j + out_second_order[2]
